...
find_treatment_combination <- function(data) {
  medicines <- c("glipizide", "insulin", "metformin", "glyburide", "glyburide.metformin", "pioglitazone", "repaglinide", "nateglinide", "rosiglitazone", "acarbose", "glimepiride")
  
  combinations <- apply(data[,medicines], 1, function(row) {
    sorted_medicines <- sort(medicines[row == 1])
...
```{r}
logit <- glm(y ~ . -encounter_id - patient_nbr - admission_type_id - discharge_disposition_id - readmitted, data = trainData, 
             family = "binomial", na.action = na.omit)
summary(logit)
mod_logit2 <- glm(y ~ . -encounter_id - patient_nbr - admission_type_id - discharge_disposition_id - readmitted, data = trainDataMod, 
             family = "binomial", na.action = na.omit)
summary(mod_logit2)
```
...
```{r}
confusionMatrix(testDataMod$y, as.factor(ifelse(predict(mod_logit2, testDataMod, type="response")> 0.5, 1, 0)))
```
```{r}
par(pty="s")
